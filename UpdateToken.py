...
def getmstoken(ms_token,appnum):
    ...
    for retry_ in range(4):
        ...
        #json.dumps失败
        if html.status_code < 300:
            print(r'账号/应用 '+str(appnum)+' 的refresh_token获取成功')
            break
        else:
            if retry_ == 3:
                print(r'账号/应用 '+str(appnum)+' refresh_token获取失败'+'\n'+'请检查secret里 CLIENT_ID , CLIENT_SECRET , MS_TOKEN 格式与内容是否正确，然后重新设置')
    jsontxt = json.loads(html.text)
...
def setsecret(encrypted_value,key_id,puturl,appnum):
    ...
    for retry_ in range(4):
        ...
        if putstatus.status_code < 300:
            print(r'账号/应用 '+str(appnum)+' 的refresh_token上传成功')
            break
        else:
            if retry_ == 3:
                print(r'账号/应用 '+str(appnum)+' 的refresh_token上传失败，请检查secret里 GH_TOKEN 格式与设置是否正确')        
    return putstatus
